...
uniform vec4 uCustom;	//	uCustom.x = gamma correct, 
						//	uCustom.y = amount of direct transfer of USE_BASE_COLOR_OPACITY_MAP (0.0 = classic PBR, 1.0 = USE_BASE_COLOR_OPACITY_MAP),
						//	uCustom.z = opacity, 
uniform vec4 uFade;		//	uFade.x = master opacity

// We hijacked the ambient color components to control the fade in/fade out of the whole screen using uFogColor
// see : uAmbientColor
...
void main() {
	...
	color = mix(color, base_opacity.xyz, uCustom.y);
	...
	color = pow(color, vec3_splat(1. / uCustom.x));
#endif // FORWARD_PIPELINE_AAA != 1
vec3 fade_color;
#if FORWARD_PIPELINE_AAA != 1
	fade_color = uFogColor.xyz; 
#else
	fade_color = pow(uFogColor.xyz, vec3_splat(uCustom.x)); // compensate the gamma correct
#endif
	color = mix(color, fade_color, uAmbientColor.x);
	gl_FragColor = vec4(color, opacity * uCustom.z * uFade.x);
