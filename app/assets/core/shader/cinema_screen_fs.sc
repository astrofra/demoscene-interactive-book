...
void main() {
	//
vec4 base_opacity = vec4(0.0, 0.0, 0.0, 1.0);

#if DEPTH_ONLY != 1
	vec4 occ_rough_metal = vec4(0.0, 0.0, 0.0, 0.0);
	//
#if USE_SELF_MAP
	...
	#define SELF_POW 6.5
	#define SELF_MULTIPLIER 20.0
	self = SELF_MULTIPLIER * vec4(pow(self.x, SELF_POW), pow(self.y, SELF_POW), pow(self.z, SELF_POW), self.w);
